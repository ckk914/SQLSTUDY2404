...
SELECT 
    emp_no, 
    TO_CHAR(SUM(pay_amt), 'L999,999,999') "사원별 총급여액",                            --형식 ￦97,760,000
    TO_CHAR(ROUND(AVG(pay_amt), 2), 'L999,999,999') "사원별 평균급여액",      --￦형식 4,073,333
...
SELECT 
    ...
    FROM TB_EMP
 GROUP BY DEPT_CD,SEX_CD        --성별과 부서로 그룹화 (남자,부서1) (여자,부서1) 이런식으로 묶임.!
 ORDER BY DEPT_CD
;
-------------------------------------------------
--사원별로 2019년에 급여 평균액, 
...
SELECT 
    ...
    , dept_cd
FROM tb_emp
ORDER BY dept_cd, emp_nm DESC  --dept_cd 는 오름차순 , emp_nm 는 내림차순
;
--------------------------------------------------------------------------------------------------------------
SELECT 
    emp_no AS 사번
...
SELECT 
    ...
    , dept_cd
FROM tb_emp
ORDER BY 3 ASC, 1 DESC  -- = dept_cd ASC , emp_no DESC
;
--------------------------------------------------------------------------------------------------------------
SELECT 
    emp_no
    ...
    , dept_cd
FROM tb_emp
ORDER BY 3 ASC, emp_no DESC   --숫자 : 속성 순서  dept_cd ASC, emp_no DESC
;
--------------------------------------------------------------------------------------------------------------
...
SELECT 
    ...
    , SUM(pay_amt) 연봉
FROM tb_sal_his
WHERE pay_de BETWEEN '20190101' AND '20191231'  --1. 해당 내용으로 먼저 분류
GROUP BY emp_no
HAVING AVG(pay_amt) >= 4500000                                --2. 함수 돌리고 나중에 한번 더 분류
ORDER BY SUM(pay_amt) DESC                                       --3. 마지막 정렬
;
--------------------------------------------------------------------------------------------------------------
--null이 제일 크다 (오라클에서는)
SELECT
