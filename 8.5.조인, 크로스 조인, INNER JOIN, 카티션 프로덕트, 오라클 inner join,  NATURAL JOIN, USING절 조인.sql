...
;
------------------------------------------------------------
--사원의 사원번호와 취득한 자격증명을 조회  (오라클 inner join)
-----------------------------------------------------------------
SELECT
A.EMP_NO,
A.EMP_NM,
...
;
-----------------------------------------------------------------
--사원의 사원번호와 취득한 자격증명,부서명 을 조회 (오라클 inner join)
-----------------------------------------------------------------
SELECT
A.EMP_NO,
A.EMP_NM,
...
-- FROM 테이블 
       --테이블, 테이블 에서 , (콤마)대신  JOIN을 넣고
       -- 줄을 바꾸고 ON을 쓰고 조인 조건을 입력 
---------------------------------------------------------------------------------------------------------------
SELECT
    E.emp_no, E.emp_nm, D.dept_nm, C.certi_nm, EC.acqu_de
FROM tb_emp E 
INNER JOIN tb_dept D 
ON E.dept_cd = D.dept_cd  --E와 D 조건
JOIN tb_emp_certi EC
ON E.emp_no = EC.emp_no  --E와 EC 조건
INNER JOIN tb_certi C 
ON C.certi_cd = EC.certi_cd   --C와 EC조건
...
-- GROUP BY  열을 그룹화
-- HAVING    그룹화 조건
-- ORDER BY  정렬할 열 [ASC | DESC];      ASC: 오름차순,DESC내림차순               
----------------------------------------------------------------
...
------------------------------------------------------------------------
--내츄럴 조인이 알아서 공통 속성에 대해 스마트하게 조인해준다
------------------------------------------------------------------------
--SELECT할때, => ★dept_cd 앞에 식별자 떼어내야함~! 식별자는 한번만 써주기.!★
SELECT 
...
------------------------------------------------------------------------
--  1. NATURAL조인에서는 
--   자동으로 이름과 타입이 일치하는 모든 컬럼에 대해
--    조인이 일어나지만 

--USING을 사용하면
-- 원하는 컬럼에 대해서면 선택적 조인조건을 
--    부여할 수 있습니다.
--  2. USING절에서도 조인 컬럼에 대해 ALIAS나 테이블명을 표기하시면 안됩니다.
------------------------------------------------------------------------
--★ USING() 쓸때도 식별자 제거하고 속성만 적기!
   -- SELECT문 쓸때 -> 식별자 제거하고 dept_md로만 써야함★
SELECT 
   A.emp_no, A.emp_nm, dept_cd, B.dept_nm
